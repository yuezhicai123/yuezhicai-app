...
### v1.1.6 (2025-12-28) - 消息通知功能

- 📢 **新增消息通知功能**：实现了完整的Android通知系统，支持接收和显示应用消息
- 🛡️ **通知权限管理**：根据Android版本自动请求必要的通知权限
- 🔔 **通知渠道配置**：创建了默认通知渠道，确保在Android 8.0+上能正常显示通知
- ⚙️ **通知服务**：实现了通知服务和广播接收器，用于管理通知的发送和接收
- 🎨 **优化的通知体验**：点击通知可以直接打开应用，提供流畅的用户体验

### v1.1.5 (2025-12-28) - 权限与相机功能优化

- 📸 **增强相机拍摄功能**：修复并优化了WebView中的相机拍摄功能，现在可以直接使用相机拍摄照片并上传
- 🛡️ **完善FileProvider支持**：添加了FileProvider配置，确保在Android 7.0+上能安全访问相机拍摄的照片
- 🔧 **优化权限请求逻辑**：修复了权限请求和处理的相关问题，确保文件访问和相机权限能正确获取
- 📁 **改进文件选择器**：优化了文件选择器逻辑，支持在一个界面中选择文件或使用相机拍摄
- 🐛 **修复构建错误**：解决了所有编译错误，确保应用能正常构建和运行
